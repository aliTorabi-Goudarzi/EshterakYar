...
enum class Currency(
    ...
    IRT("IRT", "تومان", "ت", 1.0),
    IRR("IRR", "ریال", "ر", 0.1), // هر ۱۰ ریال = ۱ تومان
 ...
 * ابزار تبدیل ارزها
 *
 * این کلاس از نرخ‌های ثابت استفاده می‌کند. در نسخه‌های آینده می‌توان از API برای نرخ‌های زنده
 * استفاده کرد.
 */
object CurrencyConverter {
    ...
    fun formatPrice(
            amount: Double,
            currency: Currency,
            showDecimals: Boolean = currency != Currency.IRT && currency != Currency.IRR
    ): String {
        val formattedAmount =
                if (showDecimals) {
        ...
        // برای تومان و ریال، نماد را بعد از عدد قرار می‌دهیم (فرمت فارسی)
        return if (currency == Currency.IRT || currency == Currency.IRR) {
