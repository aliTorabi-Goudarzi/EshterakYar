...
interface SubscriptionDao {
    ...
    @Query("SELECT * FROM subscriptions WHERE category = :category ORDER BY nextRenewalDate ASC")
    fun getSubscriptionsByCategory(category: String): Flow<List<Subscription>>
    
    @Query("SELECT * FROM subscriptions WHERE nextRenewalDate BETWEEN :startDate AND :endDate ORDER BY nextRenewalDate ASC")
    fun getSubscriptionsByRenewalDateRange(startDate: Long, endDate: Long): Flow<List<Subscription>>
    
    @Query("SELECT * FROM subscriptions WHERE nextRenewalDate <= :reminderDate AND isActive = 1 ORDER BY nextRenewalDate ASC")
